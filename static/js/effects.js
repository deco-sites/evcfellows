(function () {
  var on = addEventListener,
    ...
    client = function () {
      ...
      if (
        o.os == "mac" && ("ontouchstart" in window) &&
        ...
        f = function (s) {
          ...
          for (i = 0; i < r.length; i++) {
            if (
              ...
              r[i] instanceof CSSStyleRule && r[i].selectorText == selectorText
            ) a.push(r[i]);
          }
        },
        x,
    ...
    loadElements = function (parent) {
      ...
      for (i = 0; i < a.length; i++) if (a[i].paused) a[i].play();
    ...
    unloadElements = function (parent) {
      ...
      for (i = 0; i < a.length; i++) if (!a[i].paused) a[i].pause();
      e = $(":focus");
      if (e) e.blur();
    };
      ...
      scrollEvents.items.forEach(function (item) {
        ...
        if (item.initialState !== null) {
          ...
          item.initialState = null;
            ...
            case 2:
              a = top + (height * 0.5);
              state = a > (elementTop - item.offset) &&
            ...
            case 3:
              ...
              if (
                b + (height * 0.375) >= document.body.scrollHeight - scrollPad
              ) b = document.body.scrollHeight + scrollPad;
              state = b > (elementTop - item.offset) &&
                a < (elementBottom + item.offset);
              break;
            case 4:
              ...
              if (
                (viewportBottom - viewportTop) >= (elementBottom - elementTop)
              ) {
                state =
  ...
  var onvisible = {
    effects: {
      ...
      "zoom-in": {
        ...
        transition: function (speed, delay) {
          return "opacity " + speed + "s ease" +
            (delay ? " " + delay + "s" : "") + ", " + "transform " + speed +
            "s ease" + (delay ? " " + delay + "s" : "");
        },
        rewind: function (intensity, alt) {
          this.style.opacity = 0;
          this.style.transform = "scale(" +
            (1 - ((alt ? 0.25 : 0.05) * intensity)) + ")";
        },
        play: function () {
          this.style.opacity = 1;
      ...
      "tilt-left": {
        ...
        rewind: function (intensity, alt) {
          this.style.opacity = 0;
          this.style.transform = "rotate(" + ((alt ? 45 : 5) * intensity) +
            "deg)";
        },
        play: function () {
          this.style.opacity = 1;
      ...
      "pop-in": {
        ...
        keyframes: function (intensity) {
          ...
          return [{ opacity: 0, transform: "scale(" + (1 - diff) + ")" }, {
            opacity: 1,
      ...
      "bounce-up": {
        ...
        keyframes: function (intensity) {
          ...
          return [{ opacity: 0, transform: "translateY(" + diff + "rem)" }, {
            opacity: 1,
      ...
      "bounce-left": {
        ...
        keyframes: function (intensity) {
          let diff = (intensity + 1) * 0.075;
          return [{ opacity: 0, transform: "translateX(" + diff + "rem)" }, {
            opacity: 1,
    ...
    add: function (selector, settings) {
      var _this = this,
        ...
        stagger = "stagger" in settings
          ? (parseInt(settings.stagger) >= 0
            ? parseInt(settings.stagger)
            : false)
          : false,
        staggerOrder = "staggerOrder" in settings
          ? settings.staggerOrder
          : "default",
        staggerSelector = "staggerSelector" in settings
          ? settings.staggerSelector
          : null,
      ...
      $$(selector).forEach(function (e) {
        ...
        children = (stagger !== false && staggerSelector)
          ...
          enter: children
            ? function () {
              var staggerDelay = 0,
                ...
                a;
              if (staggerOrder == "default") children.forEach(childHandler);
              else {
  ...
  onvisible.add(".text-component.style-1", {
    style: "fade-up",
    speed: 750,
    intensity: 0,
    threshold: 3,
  ...
  onvisible.add(".buttons-component.style-1", {
    style: "fade-up",
    speed: 750,
    intensity: 0,
    threshold: 3,
  ...
  onvisible.add(".text-component.style-2", {
    style: "fade-left",
    speed: 750,
    intensity: 0,
    threshold: 3,
  ...
  onvisible.add(".text-component.style-3", {
    style: "fade-left",
    speed: 750,
    intensity: 0,
    threshold: 3,
